...
for j = 1:numstains
    if room_size(1)-3<=Xs(j) && Xs(j)<=room_size(1)+3
        face(j) = 1; %Stains Contained on Surface #1
    elseif room_size(2)-3<=Xs(j) && Xs(j)<=room_size(2)+3
        face(j) = 3; %Stains Contained on Surface #3
    elseif room_size(3)-3<=Zs(j) && Zs(j)<=room_size(3)+3
        face(j)=4; %Stains Contained on Surface #4
    elseif room_size(4)-3<=Zs(j) && Zs(j)<=room_size(4)+3
