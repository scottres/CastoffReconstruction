...
% % % 
% % % Required Installation for GNU Octave Compatibility:
...
% % % 
% % % User Inputs: (DRIVER)
% % %  - 'x','y','z' stain coordinate locations (in centimeters) relative to user defined origin
% % %  - 'lngth','minor' major and minor axis lengths of stains in mm
% % %  - 'alpha','gamma' stain impact and directional angles
% % %  - 'room', room length (along x-dimension), width (along y-dimension), and height (along z-direction) (x,y,z) in centimeters
% % %  - 'xmin','xmax','ymin','ymax','zmin','zmax' minimum and maximum coordinate of possible region of cast-off origin)
% % %  - 'res' Spatial resolution of reconstruction (Length of Discretized Uniform Regions of Space Dimensions) (1-15cm is the recommended range) (15cm took ~30 seconds, 10cm took ~1 minute, and 7.5cm took ~1 hour in a large room with several hundred stains and default specifications)
% % %  - 'n_b','n_u','n_f','n_d','t_b','t_u','t_f','t_d' surface normal (n) and tangential (t) unit vectors (four surfaces by default are assumed to be perpendicular) (surfaces include: back,upward,front,downward) *** Does not incorporate side surfaces ***
...
% % %  - 'alpha60more' Choose '1' to Remove impact angles Alpha Values Greater Than 60 degrees; Choose '0' to Keep All Alpha Values Greater Than 60 degrees
...
% % % 
% % % User Inputs: (MAIN)
% % %  - 'percentiles' Choose Resultant Product Distribution Percentiles for Plotting Likelihood Regions (this can be change when replotting the final figure (Figure 4) in the Post-processor
% % % 
% % % Ad-hoc User Defined Variables:
% % %  - 'stdev' Approximate Standard Deviation of Stain Width Measurement in mm (0.1 is the default value)
% % %  - 'Spread_Fact_cu' Spreading Factor Uncertainty in Distance between a given Discretized Region in Space and Arc (10cm-30cm is recommended range, 2*res (cm) is the default)
...
room = [-621 364 258.5]; %Room Dimensions
...
ymin = -246.5; %min(y); %Select Minimum Y-coordinate for Area of Interest Assignment
ymax = ymin+room(2); %max(y); %Select Maximum Y-coordinate for Area of Interest Assignment
zmin = -83.5; %Select Minimum Z-coordinate for Area of Interest Assignment
...
save('Castoff_Reconstruction_DRIVER_Liscio_Trial_1_Human_Created.mat');
