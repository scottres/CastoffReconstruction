function [Psi_tot,Sn] = Castoff_Reconstruction_FUNC(face,v,aoi,Xs,Ys,Zs,alpha_p,alpha,alpha_pg,alpha_orig,gamma,minor,Ref,InOutTrajectory,InRoom,max_room_size,min_room_size,Lx,Ly,Lz,Nx,Ny,Nz,res,xmin,ymin,zmin,stdev,cu_cx,cu_cy,cu_cz,ip,isocubes,Spread_Fact_cu,Spread_Fact_upsilon,Spread_Fact_delrad,dalpha_range,dgamma_range,SF_theta_range,datamat,clstr_num,iq,comb_num,sig_n,fileID)
...
dalpha = sqrt(sum(d_alpha.^2)); %Root Sum Squared Alpha Impact Angle Uncertainty in Degrees
d_gamma = (0.4204*exp(0.0541*alpha_orig*180/pi))*pi/180; %Gamma Impact Angle Uncertainty in Degrees
dgamma = sqrt(sum((d_gamma.^2))); %Root Sum Squared Gamma Impact Angle Uncertainty in Degrees
...
Psi_tot = Psi_cu.*Psi_theta.*Psi_upsilon*((1/(Spread_Fact_delrad*sqrt(2*pi)))*exp((-1/2)*(del_rad/Spread_Fact_delrad)^2));
