...
d_alpha(d_alpha>45*pi/180)=45*pi/180;
...
% Determine Theta Spreading Factor
for vui = 1:numstains
    Vu_1 = [cos(d_alpha(vui)),-sin(d_alpha(vui));sin(d_alpha(vui)),cos(d_alpha(vui))]*[Vu(vui,1);Vu(vui,3)];
    Vu_2 = [cos(-d_alpha(vui)),-sin(-d_alpha(vui));sin(-d_alpha(vui)),cos(-d_alpha(vui))]*[Vu(vui,1);Vu(vui,3)];
    slopevu1 = Vu_1(2)/Vu_1(1);
    slopevu2 = Vu_2(2)/Vu_2(1);
    vupt = [XYZu(vui,1), XYZu(vui,3)];
    vint1 = vupt(2)-slopevu1*vupt(1);
    vint2 = vupt(2)-slopevu2*vupt(1);
    sfb = [center(1), center(3)];
    sfa = sfb+[100 0];
    [sf1cx sf1cz] = linecirc(slopevu1,vint1,center(1),center(3),radius);
    [sf2cx sf2cz] = linecirc(slopevu2,vint2,center(1),center(3),radius);
    sf1ca = [sf1cx(1), sf1cz(1)];
    sf1cb = [sf1cx(2), sf1cz(2)];
    sf2ca = [sf2cx(1) sf2cz(1)];
    sf2cb = [sf2cx(2) sf2cz(2)];
    nsfa1a = (sf1ca-sfb)/norm(sf1ca-sfb);
    nsfa1b = (sf1cb-sfb)/norm(sf1cb-sfb);
    nsfb1 = (sfa-sfb)/norm(sfa-sfb);
    nsfa2a = (sf2ca-sfb)/norm(sf2ca-sfb);
    nsfa2b = (sf2cb-sfb)/norm(sf2cb-sfb);
    nsfb2 = (sfa-sfb)/norm(sfa-sfb);
    veta1a = atan2(norm(det([nsfa1a;nsfb1])),dot(nsfa1a,nsfb1));
    veta1b = atan2(norm(det([nsfa1b;nsfb1])),dot(nsfa1b,nsfb1));
    veta2a = atan2(norm(det([nsfa2a;nsfb2])),dot(nsfa2a,nsfb2));
    veta2b = atan2(norm(det([nsfa2b;nsfb2])),dot(nsfa2b,nsfb2));
    
    if (lin1 <= veta1a) && (veta1a <= lin2) %abs(theta1(sj)-theta2(sj)) <= eta(sj) && eta(sj) <= abs(theta1(sj)-theta2(sj))
       SF_theta1a(vui) = lin2-veta1a;
    elseif (lin2 <= veta1a) && (veta1a <= lin1)
       SF_theta1a = lin1-veta1a;
       SF_theta1a(vui) = mod(SF_theta1,(2*pi));
    else
       SF_theta1a(vui) = NaN;
    end
    
    if (lin1 <= veta1b) && (veta1b <= lin2) %abs(theta1(sj)-theta2(sj)) <= eta(sj) && eta(sj) <= abs(theta1(sj)-theta2(sj))
       SF_theta1b(vui) = lin2-veta1b;
    elseif (lin2 <= veta1b) && (veta1b <= lin1)
       SF_theta1b = lin1-veta1b;
       SF_theta1b(vui) = mod(SF_theta1,(2*pi));
    else
       SF_theta1b(vui) = NaN;
    end
    
    if (lin1 <= veta2a) && (veta2a <= lin2)
       SF_theta2a(vui) = lin2-veta2a;
    elseif (lin2 <= veta2a) && (veta2a <= lin1)
       SF_theta2a = lin1-veta2a;
       SF_theta2a(vui) = mod(SF_theta2,(2*pi));
    else
       SF_theta2a(vui) = NaN;
    end
    
    if (lin1 <= veta2b) && (veta2b <= lin2)
       SF_theta2b(vui) = lin2-veta2b;
    elseif (lin2 <= veta2b) && (veta2b <= lin1)
       SF_theta2b = lin1-veta2b;
       SF_theta2b(vui) = mod(SF_theta2,(2*pi));
    else
       SF_theta2b(vui) = NaN;
    end
end
    
SF_theta = ([SF_theta1a; SF_theta1b; SF_theta2a; SF_theta2b]);
SF_theta = SF_theta(:);
SF_theta(any(isnan(SF_theta)==1,2),:) = []; %Remove NaNs
...
if Spread_Fact_theta<SF_theta_range(1)
  Spread_Fact_theta = SF_theta_range(1); %Theta Spreading Factor Floor
elseif Spread_Fact_theta>SF_theta_range(2)
  Spread_Fact_theta = SF_theta_range(2); %Theta Spreading Factor Floor
else
...
if Spread_Fact_theta >= SF_theta_range(1) && Spread_Fact_theta < SF_theta_range(2)
  fprintf(fileID, '%s\r\n', strcat('Spread_Fact_theta (deg) =','',num2str(round(Spread_Fact_theta*180/pi)))); %Inputs are within Predefined Range
else
  fprintf(fileID, '%s\r\n', 'WARNING: The inputted value for "Spread_Fact_theta" is not ideal.'); %Inputs are within Predefined Range
  fprintf(fileID, '%s\r\n', strcat('Spread_Fact_theta (deg) =','',num2str(round(Spread_Fact_theta*pi/180)),' (ideal "Spread_Fact_theta" Range =','',num2str(SF_theta_range))); %Inputs are within Predefined Range
end
...
%     if Spread_Fact_upsilon<SF_upsilon_range(1)
%   Spread_Fact_upsilon = SF_upsilon_range(1); %Theta Spreading Factor Floor
% end

if Spread_Fact_upsilon>SF_upsilon_range(2)
  Spread_Fact_upsilon = SF_upsilon_range(2); %Theta Spreading Factor Floor
elseif Spread_Fact_upsilon<SF_upsilon_range(1)
    Spread_Fact_upsilon = SF_upsilon_range(1); %Theta Spreading Factor Floor
else
...
%Weight_delta=1;
%Psi_tot = Psi_cu.*Psi_theta.*Psi_upsilon;
Psi_tot = Psi_cu; %.*Psi_theta;
% sum(Psi_cu)
% sum(Psi_theta)
% sum(Psi_upsilon)
...
Psi_tot(any(Psi_tot<(realmin('double')^(1/clstr_num)),2)) = realmin('double')^(1/clstr_num); %Psi_tot(any(Psi_tot<(max(Psi_tot)*exp(-0.5*(sig_n^2))),2)) = max(Psi_tot)*exp(-0.5*(sig_n^2));
...
dummy = 22;%     ind(noah,:) = sub2ind([Nz Nx Ny],sub_row(noah,:),sub_col(noah,:),sub_sht(noah,:));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
