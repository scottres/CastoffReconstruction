...
for cu3 = 1:numel(cu_cx)
    ...
    Spread_Fact_cu(cu3,:) = sqrt(sum((dalpha*dist_cu(cu3))^2+(dgamma*dist_cu(cu3))^2)); %Spread_Fact_cu = res*2; %Spreading Factor %Uncertainty in Distance between a given Cube and Arc in centimeters
    if Spread_Fact_cu(cu3,:)<SF_Cu_Floor
      Spread_Fact_cu(cu3,:) = SF_Cu_Floor; %Theta Spreading Factor Floor
    end
    
%     if Spread_Fact_cu(cu3,:) >= SF_cu_range(1) && Spread_Fact_cu(cu3,:) < SF_cu_range(2)
...
%     end
    ...
    Psi_cu(cu3,:) = (1/(Spread_Fact_cu(cu3)*sqrt(2*pi)))*exp((-1/2)*(dist_cu(cu3)/(Spread_Fact_cu(cu3))).^2); %Weight to be Added to Each Cube
