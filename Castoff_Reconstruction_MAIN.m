...
data = 'Daniel_2_INPUT_DRIVER.mat';
%data = 'Daniel_2_INPUT_single triplet_DRIVER.mat';
% data = 'two_circles_input_DRIVER.mat';
% data = 'FARO_Trial_10_INPUT_DRIVER.mat';
%data = 'Ink_Trial_INPUT_DRIVER.mat';
%data = 'Swineblood_Trial_INPUT_DRIVER.mat';


%data = 'Daniel_2_INPUT_single triplet_DRIVER_05mm.mat';
load(data); %'Castoff_Reconstruction_DRIVER_SWINEBLOOD.mat' OR 'Castoff_Reconstruction_DRIVER_LISCIO_TRIAL_2_UPDATED.mat'
Spread_Fact_upsilon = 30*pi/180; %Spreading Factor %Uncertainty in Off-plane Angle (Upsilon) in radians 
Spread_Fact_delrad = 10*pi/180; %Spreading Factor %Uncertainty in Angle Measurement (del_rad) in radians
SF_Theta_Floor = NaN; %Theta Spreading Factor Floor (degrees)
SF_Cu_Floor = NaN; %Theta Spreading Factor Floor (cm)
sig_n = 2; %Minimum PDF Value Standard Deviations
...
SF_cu_range = [0,20]; %Distance Spreading Factor Allowable Range, in cm (zero to 5cm or 30 cm give reasonable results)
SF_theta_range = [0,10*pi/180]; %In-plane Angle Spreading Factor Allowable Range (zero to 1 or 20 degree give reasonable results)
SF_upsilon_range = [0,pi]; %Off-plane Angle Spreading Factor Allowable Range
SF_delrad_range = [0,20*pi/180]; %Angle Measurement Spreading Factor Allowable Range
...
if alpha_clstr == 1;
    for ac = 1:length(clstr_alpha);
        Alpha_gn = combvec2(alpha_gn',alpha_gn')'; %All Possible Combinations of Two Global Alpha Angles
        ALPHA_GN = combvec2(Alpha_gn',Alpha_gn(:,1)')'; %All Possible Combinations of Three Global Alpha Angles
        pgn_ind = combvec2(1:numstains,1:numstains)'; %All Possible Combinations of Two Stain Indices
        PGN_IND = combvec2(pgn_ind',pgn_ind(:,1)')'; %All Possible Combinations of Three Stain Indices
...
if dist_clstr == 1 && dwn_samp_stains ~= 1;
    for ad = 1:length(clstr_dist);
        ...
        R_dist = combvec2(r_dist',r_dist')'; %All Possible Combinations and Permutations of Two Sets of Stain Coordinates
        R_DIST = combvec2(R_dist',R_dist(:,1:3)')'; %All Possible Combinations and Permutations of Three Sets of Stain Coordinates
        ...
        pgn_ind = combvec2(1:numstains,1:numstains)'; %All Possible Combinations and Permutations of Two Sets of Stain Indices
        PGN_IND = combvec2(pgn_ind',pgn_ind(:,1)')'; %All Possible Combinations and Permutations of Three Sets of Stain Indices
...
for iq = 1:comb_num;
    ...
    for ip = 1:clstr_num(iq); %Number of Clusters
        ...
        [Weight,Sn] = Castoff_Reconstruction_FUNC(Face,V,aoi,XS,YS,ZS,Alpha_p,Alpha,Alpha_pg,Alpha_orig,Gamma,Minor,Ref,InOutTrajectory,InRoom,max_room_size,min_room_size,Lx,Ly,Lz,Nx,Ny,Nz,res,xmin,ymin,zmin,stdev,cu_cx,cu_cy,cu_cz,ip,isocubes,Spread_Fact_upsilon,Spread_Fact_delrad,dalpha_range,dgamma_range,SF_cu_range,SF_theta_range,SF_upsilon_range,datamat,clstr_num,iq,comb_num,sig_n,fileID); %Function Determining Castoff Reconstruction
        if ishandle(3)
          figure(3);
          hold on;
    ...
    Current_min = min(min(min(isocubes,[],1),[],2),[],3)
    if ip == 1;
...
%     results{iq,9} = FacE; %Save Results
   ...
   hold off;
    ...
    [f1,v1] = isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(1)^NUM_clstr)/maxcu); %Save Distribution Vertices of First Percentile
    [f2,v2] = isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(2)^NUM_clstr)/maxcu); %Save Distribution Vertices of Second Percentile
    [f3,v3] = isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(3)^NUM_clstr)/maxcu); %Save Distribution Vertices of Third Percentile
...
if isempty(nonzeros(isonorm-ones(size(isonorm)))) == 1 || ~(any(isonorm(:) < (percentiles(1)^NUM_clstr)/maxcu));
...
elseif ~(any(isonorm(:) < (percentiles(2)^NUM_clstr)/maxcu));
   p5 = patch(isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(1)^NUM_clstr)/maxcu,'FaceColor',cell2mat(colorcu(1)),'EdgeAlpha',transcu(1),'FaceAlpha',transcu(1));
   Vol_Region_1 = (sum(isonorm(:) > percentiles(1)^NUM_clstr)/maxcu)*res^3
...
elseif ~(any(isonorm(:) < (percentiles(3)^NUM_clstr)/maxcu));
   p5 = patch(isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(1)^NUM_clstr)/maxcu,'FaceColor',cell2mat(colorcu(1)),'EdgeAlpha',transcu(1),'FaceAlpha',transcu(1));
   Vol_Region_1 = (sum(isonorm(:) > percentiles(1)^NUM_clstr)/maxcu)*res^3
   p6 = patch(isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(2)^NUM_clstr)/maxcu,'FaceColor',cell2mat(colorcu(2)),'EdgeAlpha',transcu(3),'FaceAlpha',transcu(2));
   Vol_Region_2 = (sum(isonorm(:) > percentiles(2)^NUM_clstr)/maxcu)*res^3
   ...
   p6 = patch(isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(2)^NUM_clstr)/maxcu,'FaceColor',cell2mat(colorcu(2)),'EdgeAlpha',transcu(3),'FaceAlpha',transcu(2));
   Vol_Region_2 = (sum(isonorm(:) > percentiles(2)^NUM_clstr)/maxcu)*res^3
   p7 = patch(isosurface(Xcu,Ycu,Zcu,isonorm,(percentiles(3)^NUM_clstr)/maxcu,'FaceColor',cell2mat(colorcu(3)),'EdgeAlpha',transcu(4),'FaceAlpha',transcu(3));
   Vol_Region_3 = (sum(isonorm(:) > percentiles(3)^NUM_clstr)/maxcu)*res^3
