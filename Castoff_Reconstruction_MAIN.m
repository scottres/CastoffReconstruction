...
for iq = 1:comb_num;
    ...
    for ip = 1:clstr_num(iq); %Number of Clusters
        ...
        [Weight,Sn] = Castoff_Reconstruction_FUNC(Face,V,aoi,XS,YS,ZS,Alpha_p,Alpha,Alpha_pg,Alpha_orig,Gamma,Minor,Ref,InOutTrajectory,InRoom,max_room_size,min_room_size,Lx,Ly,Lz,Nx,Ny,Nz,res,xmin,ymin,zmin,stdev,cu_cx,cu_cy,cu_cz,ip,isocubes,Spread_Fact_cu,Spread_Fact_upsilon,Spread_Fact_delrad,dalpha_range,dgamma_range,SF_theta_range,datamat,clstr_num,iq,comb_num,sig_n,fileID); %Function Determining Castoff Reconstruction
        if ishandle(3)
          figure(3);
          hold on;
